<!--
  목적 : 검진기관 기준정보 - 선택검진(탭)
  Detail : 검진기관별(검진유형별 검사종류별) 선택검진항목 등록/수정화면
  *
  examples:
  *
  -->
  ...
  <b-container fluid>
    ...
    <b-row class="mt-3" ref="editBox">
      <b-col sm="12">
        <b-row>
          <b-col sm="12">
            <y-label label="검진종류 - 선택 검진항목 상세" icon="user-edit" color-class="cutstom-title-color" />
          </b-col>
        </b-row>
        <b-card>
          <b-row>
              ...
              <b-row>
                <b-col sm="6" md="6" lg="6" xl="6" class="col-xxl-3">
                  <y-select
                    ...
                    v-model="checkupOrgOptional.heaCheckupOrgNo"
                    v-validate="'required'"
                ...
                <b-col sm="6" md="6" lg="6" xl="6" class="col-xxl-3">
                  ...
                  label="검진년도"
                  name="remark"
                  v-model="checkupOrgOptional.year"
                  v-validate="'required'"
                ...
                <b-col sm="12" md="12" lg="12" xl="12" class="col-xxl-6">
                  <y-select
                    ...
                    v-model="checkupOrgOptional.heaCheckupTypeCd"
                    v-validate="'required'"
...
export default {
  ...
  watch: {
     ...
     */
    'checkupOrgOptional.heaCheckupOrgNo': {
      handler: function (value, oldVal) {
     ...
     */
    'checkupOrgOptional.heaCheckupTypeCd': {
      handler: function (value, oldVal) {
        this.setSelectedTestItem(null); 
      },
    },
  ...
  methods: {
     ...
     */
    getComboTestItems () {
      if (this.checkupOrgOptional.heaCheckupOrgNo <= 0)
      {
      ...
      else if (!this.checkupOrgOptional.year) 
      {
      ...
      else if (!this.checkupOrgOptional.heaCheckupTypeCd) 
      {
      ...
      this.searchTestItemParam.heaCheckupOrgNo = this.checkupOrgOptional.heaCheckupOrgNo;
      this.searchTestItemParam.year = this.$_.clone(this.$comm.moment(this.checkupOrgOptional.year).format('YYYY'));
      this.searchTestItemParam.heaCheckupTypeCd = this.checkupOrgOptional.heaCheckupTypeCd;
      this.searchTestItemParam.useYn = 'Y';
      this.searchTestItemParam.optionalYn = 'N' // 공통검진에 저장된 항목들만 제거한다.
    ...
    setSelectedTestItem (data) {
      ...
      {
        data = {
          year: this.checkupOrgOptional.year,
          heaCheckupTypeCd: this.checkupOrgOptional.heaCheckupTypeCd,
          heaCheckupOrgNo: this.checkupOrgOptional.heaCheckupOrgNo,
          heaTestClassCd: this.checkupOrgOptional.heaTestClassCd,
        };
      }
      ...
      if (data.heaCheckupOrgNo > 0
        ...
        this.$http.request((_result) => {
          if (_result.data.length > 0)
          {
            ...
            this.editable = true;
            this.checkupOrgOptional = this.$_.clone(_result.data[0]);
    ...
    beforeSubmit () {
      ...
      this.checkupOrgOptional.deleteHeaCheckupTestItems = [];
      this.checkupOrgOptional.insertHeaCheckupTestItems = [];
      this.$validator.validateAll().then((_result) => {
        ...
        if (_result)
        {
          ...
          this.$_.forEach(receiveData, (_item) => {
            ...
            {
              this.checkupOrgOptional.deleteHeaCheckupTestItems.push(_item);
            }
          });
          ...
          this.$_.forEach(this.$_.map(this.gridTestItemLeftOptions.data, 'heaTestItemCd'), (_item) => {
            ...
            {
              this.checkupOrgOptional.insertHeaCheckupTestItems.push(_item);
            }
          });
          this.checkupOrgOptional.year = this.$_.clone(this.$comm.moment(this.checkupOrgOptional.year).format('YYYY'));
    ...
    beforeInsert () {
      this.$validator.validateAll().then((_result) => {
        ...
        if (_result)
        {
          ...
          this.checkupOrgOptional.year = this.$_.clone(this.$comm.moment(this.checkupOrgOptional.year).format('YYYY'));
          this.checkupOrgOptional.heaTestItemCd = this.$_.map(this.gridTestItemLeftOptions.data, 'heaTestItemCd').toString();
    ...
    /** button 관련 이벤트 **/
    /**
     * 검진종류 - 선택 검진항목 목록 조회 및 선택된 검진항목 목록 리스트 저장
     *  수정 버튼 callback
    ...
    btnClearClickedCallback () {
      ...
      this.disabled = false;
      this.insertable = true;
      Object.assign(this.$data.checkupOrgOptional, this.$options.data().checkupOrgOptional);
    ...
    /**
     * 검색 조건에 따른 데이터 조회 후 검진종류 - 선택 검진항목 목록에 리스트 적용
     * _result : backend에서 return하는 데이터
     */
