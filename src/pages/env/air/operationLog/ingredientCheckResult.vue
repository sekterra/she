...
export default {
  ...
  watch: {
    isSaveIngredientCheckResult: function (newValue, oldValue) {
      if (this.isSaveIngredientCheckResult) {
  ...
  methods: {
    init () {
      ...
      this.gridOptions.header = [
        { text: '원료명', name: 'eairIngrNm', width: '160px', align: 'center', sortable: false },
      ...
      this.editUrl = transactionConfig.env.air.operationLog.ingredientCheckResult.edit.url;
      this.detailUrl = selectConfig.env.air.operationLog.ingredientCheckResult.get.url;
    ...
    beforeUpdateSubmit () {
      ...
      this.$validator.validateAll().then((_result) => {
        if (_result) {
          if (this.isSaveIngredientCheckResult) this.isUpdateSubmit = true;
          else {
            window.getApp.$emit('CONFIRM', {
              title: '확인',
              message: '원료사용량 정보를 저장 하시겠습니까?',
              type: 'info',
              confirmCallback: () => {
    ...
    btnUpdateClickedCallback (_result) {
      ...
      this.getList(); 
      if (this.isSaveIngredientCheckResult) this.$emit('callbackSaveOperationLog', { 'isSaveIngredientCheckResult': true });
      else {
        window.getApp.$emit('ALERT', {
          title: '안내',
          message: '원료사용량 정보를 정상적으로 저장하였습니다.',
          type: 'success',
        });
      }
    },
