  ...
  <b-container fluid>
    ...
    <b-row class="mt-3" ref="detailBox">
      <b-col sm="12">
        <b-row>
          <b-col sm="12">
            <y-label label="연료 등록" icon="user-edit" color-class="cutstom-title-color" />
          </b-col>
        </b-row>
        <b-card>          
          <b-row> 
...
export default {
  ...
  methods: {
    init () {
      ...
      this.insertUrl = transactionConfig.env.air.baseInfo.fuel.insert.url;
      this.editUrl = transactionConfig.env.air.baseInfo.fuel.edit.url;
      this.searchUrl = selectConfig.env.air.baseInfo.fuel.list.url;
    ...
    getDetail (data) {
      this.$http.url = this.$format(this.detailUrl, data.eairFuelCd);
      this.$http.type = 'get'; 
      this.$http.request((_result) => {
    ...
    checkDuplicate () {
      ...
      if (item != null) {
        if (this.updateMode
          && this.fuel.eairFuelCd === item.eairFuelCd) {
          return false;
        }
          
        window.getApp.$emit('ALERT', {
          title: '안내',
          message: '이미 같은 이름의 연료명이 존재합니다.',
          type: 'warning',
        });
        return true;
      }
    ...
    btnClearClickedCallback (_result) {
      ...
      this.fuel.useYn = 'Y';
      this.fuel.createUserId = '';
      this.fuel.createUserNm = '';
      this.fuel.createDt = '';
      this.fuel.updateUserId = '';
      this.fuel.updateUserNm = '';
      this.fuel.updateDt = '';
    },
