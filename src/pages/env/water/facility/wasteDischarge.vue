  ...
  <b-container fluid>
    ...
    <b-row class="mt-3" ref="insertBox">
      <b-col sm="12">
        <b-row>
          <b-col sm="12">
            <y-label label="폐수배출시설 상세" icon="user-edit" color-class="cutstom-title-color" />
          </b-col>
        </b-row>
        <b-card >
          <b-row>
            ...
            <b-col sm="6" md="6" lg="6" xl="6" class="col-xxl-3">
              ...
              label="정렬순서"
              name="sortOrder"
              v-model="ewtrWasteDischFac.sortOrder"
              >
              </y-number>
            </b-col>
          </b-row>
  ...
  methods: {
    ...
    init () {
      ...
      this.gridOptions.header = [
        { text: '배출시설명', name: 'ewtrWasteDischFacNm', width: '200px', align: 'left' },
      ...
      this.editUrl = transactionConfig.env.water.facility.wasteDischFac.edit.url;
      this.insertUrl = transactionConfig.env.water.facility.wasteDischFac.insert.url;
    ...
    getDetail (data) {
      ...
      this.$http.url = this.$format(selectConfig.env.water.facility.wasteDischFac.get.url, data.ewtrWasteDischFacNo);
      this.$http.type = 'GET';
      this.$http.request((_result) => {
    ...
    /** 신규등록 하기전 UI단 유효성 검사 **/
    beforeInsert () {
      this.ewtrWasteDischFac.ewtrWasteDischFacNo = '';
    ...
    checkDuplicate () {
      ...
      if (item != null) {
        if (this.updateMode
          && this.ewtrWasteDischFac.ewtrWasteDischFacNo === item.ewtrWasteDischFacNo) {
          return false;
        }
    ...
    btnInsertClickedCallback (_result) {
      this.ewtrWasteDischFac.ewtrWasteDischFacNo = _result.data;
      this.getList();
      this.isInsert = false;
      this.updateMode = true;
      window.getApp.$emit('ALERT', {
        title: '안내',
        message: '폐수배출시설 정보를 정상적으로 저장하였습니다.',
        type: 'success',
      });
    },
    btnEditClickedCallback (_result) {
      ...
      window.getApp.$emit('ALERT', {
        title: '안내',
        message: '폐수배출시설 정보를 정상적으로 수정하였습니다.',
        type: 'success',
      });
    },
    btnClearClickedCallback () {
      Object.assign(this.$data.ewtrWasteDischFac, this.$options.data().ewtrWasteDischFac);
