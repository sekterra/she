  ...
  <b-container fluid>
    ...
    <b-row class="mt-3" ref="insertBox">
      <b-col sm="12">
        <b-row>
          <b-col sm="12">
            <y-label label="전력량계 상세" icon="user-edit" color-class="cutstom-title-color" />
          </b-col>
        </b-row>
        <b-card >
          <b-row>
            ...
            <b-col sm="6" md="6" lg="6" xl="6" class="col-xxl-3">
              ...
              label="정렬순서"
              name="sortOrder"
              v-model="ewtrPwrMeter.sortOrder"
              >
              </y-number>
            </b-col>
          </b-row>
  ...
  methods: {
    ...
    init () {
      ...
      this.gridOptions.header = [
        { text: '전력량계명', name: 'ewtrPwrMeterNm', width: '200px', align: 'left' },
    ...
    getDetail (data) {
      ...
      this.$http.url = this.$format(selectConfig.env.water.facility.pwrMeter.get.url, data.ewtrPwrMeterNo);
      this.$http.type = 'GET';
      this.$http.request((_result) => {
    ...
    /** 신규등록 하기전 UI단 유효성 검사 **/
    beforeInsert () {
      this.ewtrPwrMeter.ewtrPwrMeterNo = '';
    ...
    checkDuplicate () {
      ...
      if (item != null) {
        if (this.updateMode
          && this.ewtrPwrMeter.ewtrPwrMeterNo === item.ewtrPwrMeterNo) {
          return false;
        }
          
        window.getApp.$emit('ALERT', {
          title: '안내',
          message: '이미 같은 이름의 전력량계명이 존재합니다.',
          type: 'warning',
        });
        return true;
      }
    ...
    btnInsertClickedCallback (_result) {
      this.ewtrPwrMeter.ewtrPwrMeterNo = _result.data;
      this.getList();
      this.isInsert = false;
      this.updateMode = true;
      window.getApp.$emit('ALERT', {
        title: '안내',
        message: '전력량계 정보를 정상적으로 저장하였습니다.',
        type: 'success',
      });
    },
    btnEditClickedCallback (_result) {
      ...
      window.getApp.$emit('ALERT', {
        title: '안내',
        message: '전력량계 정보를 정상적으로 수정하였습니다.',
        type: 'success',
      });
    },
    btnClearClickedCallback () {
      Object.assign(this.$data.ewtrPwrMeter, this.$options.data().ewtrPwrMeter);
