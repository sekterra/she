  ...
  <b-container fluid>
    ...
    <b-row class="mt-3" ref="insertBox">
      <b-col sm="12">
        <b-row>
          <b-col sm="12">
            <y-label label="배출수 사용량" icon="user-edit" color-class="cutstom-title-color" />
          </b-col>
        </b-row>
        <b-card >
          <b-row>
            <b-col sm="6" md="6" lg="6" xl="6" class="col-xxl-3">
              <y-datepicker 
                ...
                label="측정일"
                name="measureYmd"
                v-model="ewtrDischChkResult.measureYmd"
                :disabled="true"
                :required="true"
                v-validate="'required'"
...
export default {
  ...
  methods: {
    ...
    init () {
      setTimeout(() => {
        this.ewtrDischChkResult.measureYmd = this.paramMeasureYmd;
      }, 200);
      ...
      this.insertUrl = transactionConfig.env.water.operationLog.dischChkResult.insert.url;
      ...
      this.$validator.validateAll().then((_result) => {
        if (_result) {
          window.getApp.$emit('CONFIRM', {
            ...
            type: 'info',
            confirmCallback: () => {
              this.ewtrDischChkResult.dischChkResultList = this.gridOptions.data;
              this.isInsert = true;
            },
          });
        }
    ...
    btnClearClickedCallback () {
      Object.assign(this.$data.ewtrDischChkResult, this.$options.data().ewtrDischChkResult);
