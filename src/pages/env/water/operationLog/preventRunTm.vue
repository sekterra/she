...
export default {
  ...
  watch: {
    isSavePreventRunTm: function (newValue, oldValue) {
      if (this.isSavePreventRunTm) {
        this.beforeUpdate();
      }
    }
  },
  ...
  methods: {
    ...
    init () {
      ...
      this.gridOptions.header = [
        { text: '오염방지시설명', name: 'ewtrPreventFacNm', width: '180px', align: 'left' },
        ...
        { text: '시간대별 근무자', name: 'shiftWorkers', width: '300px', align: 'left', type: 'text' },
      ];

      this.editUrl = transactionConfig.env.water.operationLog.preventRunTm.edit.url;
      this.searchUrl = selectConfig.env.water.operationLog.preventRunTm.list.url;
    ...
    beforeUpdate () {
      this.$validator.validateAll().then((_result) => {
        if (_result) {
          if (this.isSavePreventRunTm) this.isUpdate = true;
          else {
            window.getApp.$emit('CONFIRM', {
              title: '확인',
              message: '방지시설 가동시간 정보를 저장하시겠습니까?',
              type: 'info',
              confirmCallback: () => {
                this.isUpdate = true;
              },
    ...
    btnUpdateClickedCallback (_result) {
      this.isUpdate = false;
      if (this.isSavePreventRunTm) this.$emit('callbackSaveOperationLog', { 'isSavePreventRunTm': true });
      else {
        window.getApp.$emit('ALERT', {
          title: '안내',
          message: '방지시설 가동시간 정보를 정상적으로 저장하였습니다.',
          type: 'success',
        });
      }
      this.getList();
