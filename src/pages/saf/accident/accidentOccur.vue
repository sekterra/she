...
export default {
  ...
  methods: {
    ...
    tableLinkAccidentTitleClicked (header, data) {
      this.showAccidentPopup(data);
    },
    /**
     * 해당 사고 상세보는 팝업 open
     */
    showAccidentPopup (data) {
      this.popupOptions.target = () => import(`${'./accidentDetail.vue'}`);
      this.popupOptions.title = '사고등록';
    ...
    btnSearchCheckResultOpenPopupClicked () {
      var data = {
        ...
        accidentTitle: '', // 사고명
        accidentNum: '', // 발행번호
        accidentStepCd: 'ACCS3', // 사내사고진행단계 (계획)
        accidentYmd: '', // 발생일
        accidentHour: '', // 발생시
        accidentMinute: '', // 발생분
        deptCd: '', // 발생부서코드
        ...
        rptUserId: this.userInfo.userId, // 보고자사번
        rptUserNm: this.userInfo.userNm, // 보고자성명
        rptDeptCd: this.userInfo.deptCd, // 보고자소속코드
        rptDeptNm: this.userInfo.deptNm, // 보고자소속명
