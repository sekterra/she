        ...
        <b-col sm="12" class="px-0">
          <div slot="buttonGroup" class="float-right mb-1">
            <y-btn 
              title="신규등록"
              color="orange"
              @btnClicked="btnSearchFacilityCheckResultOpenPopupClicked" 
              @btnClickedErrorCallback="btnClickedErrorCallback"
            />
            <!-- <y-btn 
              color="red"
          ...
          <y-data-table 
            ...
            >
            <!-- v-model="facilityCheckResultValue"
              <el-table-column
                type="selection"
                slot="selection"
                align="center"
  ...
  methods: {
    init () {
      ...
      this.gridOptions.header = [
        ...
        { text: '설비유형', name: 'safFacilityTypeNm', width: '110px', },
    ...
    tableLinkCheckTitleClicked (header, data) {
      this.popupOptions.target = () => import(`${'./facilityCheckResultPlan.vue'}`);
      this.popupOptions.title = '설비점검계획';
      this.popupOptions.param = {
        safFacilityCheckResultNo: data.safFacilityCheckResultNo, // 설비점검번호
        safFacilityCheckCd: data.safFacilityCheckCd, // 설비점검종류
        safFacilityCheckSchYmd: data.safFacilityCheckSchYmd, // 설비점검예정일
        safFacilityCheckPlanNo: data.safFacilityCheckPlanNo, // 설비점검계획번호
      };
     ...
     */
    btnSearchFacilityCheckResultOpenPopupClicked () {
      this.popupOptions.target = () => import(`${'./facilityCheckResultPlan.vue'}`);
      this.popupOptions.title = '설비점검계획 등록';
      this.popupOptions.param = {
    ...
    //   if (!(this.facilityCheckResultValue && this.facilityCheckResultValue.length > 0))
    //   {
    ...
    //       this.$http.param = {
    //         'data': Object.values(this.$_.clone(this.facilityCheckResultValue))
    //       };
    //       this.$http.request((_result) => {
    //         window.getApp.$emit('ALERT', {
    //           title: '안내',
